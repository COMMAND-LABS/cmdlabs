...
# Script to attach Cursor/VSCode to the kalygo3-ai-api dev container
# Usage: ./dev-attach-ai-api.sh [cursor|code]

set -e

CONTAINER_NAME="kalygo3-embeddings-api"
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SERVICE_DIR="${PROJECT_ROOT}/kalygo3-embeddings-api"
...
if ! command -v "$EDITOR" &> /dev/null; then
    ...
    echo -e "${YELLOW}Please install $EDITOR or use the other editor.${NC}"
    exit 1
fi

echo -e "${BLUE}Attaching $EDITOR to ${CONTAINER_NAME} container...${NC}\n"
...
if ! docker ps --format "{{.Names}}" | grep -q "^${CONTAINER_NAME}$"; then
    ...
    cd "$PROJECT_ROOT"
    docker-compose -f docker-compose.dev.yml up -d ai-api
...
echo -e "${BLUE}Attaching $EDITOR to container...${NC}"
...
tell application "System Events"
    tell process "${EDITOR}"
        ...
        -- This searches for the container name
...
EOF
    ...
    echo -e "  Name: ${YELLOW}${CONTAINER_NAME}${NC}"
    echo -e "  ID (short): ${CONTAINER_ID_SHORT}"
    echo -e "  ID (full): ${CONTAINER_ID_FULL:0:20}..."
