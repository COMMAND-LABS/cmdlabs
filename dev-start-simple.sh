...
(cd "${PROJECT_ROOT}/kalygo3-embeddings-api" && pip install -r requirements.txt && uvicorn src.main:app --host 0.0.0.0 --port 9000 --proxy-headers --reload > /tmp/kalygo-embeddings-api.log 2>&1) &
...
echo "  - Embeddings API: http://localhost:9000 (log: /tmp/kalygo-embeddings-api.log)"
